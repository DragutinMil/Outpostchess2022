<template>
    <div class="chesscv">
        <div id="about-me">
            <div id="about-edit">
                <p style="padding-right: 0.625rem">About Me</p>
                <i v-if="!editable" class="far fa-edit fontawesome" @click="editable = !editable" />
            </div>
            ...
            <div>
                <button v-if="editable" class="middle2-buttons" type="button" @click="editconfirm">
                    Confirm changes
                </button>
                <button v-if="editable" class="middle2-buttons" type="button" @click="editable = !editable">
                    Cancel
                </button>
            </div>
        ...
        <p v-else class="about-text">tekst  {{user.about_me}} </p>-->
            ...
            <div>
                <p>My CV:</p>
                <div>
                    <p id="opencv" @click="newopenwindow()">
                        {{ user.cv.file_name }}
                    </p>
                    <input id="fileUpload" type="file" style="display: none" title="" @change="onCVSelected" />
                    <label for="fileUpload" class="file-input" />
                </div>
            </div>
        </div>
    </div>
